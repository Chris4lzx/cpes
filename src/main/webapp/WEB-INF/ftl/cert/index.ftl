...
<script type="text/javascript">
    ...
    var pagefunction = function() {
        ...
        if ( window.sessionStorage ) {
        	var json = window.sessionStorage.getItem("certimg/pagecfg");
        ...
        otable = $('#certimgTable').DataTable({
            ...
            "preDrawCallback" : function(settings) {
                if (!responsiveHelper_datatable_fixed_column) {
                    responsiveHelper_datatable_fixed_column = new ResponsiveDatatablesHelper($('#certimgTable'), breakpointDefinition);
                }
                
                var qurl = "${APP_PATH}/system/cert/datas.do";
                var p = "";
            ...
            "drawCallback" : function(oSettings) {
                ...
                $("#certimgTable #checkall").click(function(){
                	var flg = this.checked;
                	$.each($("#certimgTable .data-check"), function(){                	
                		this.checked = flg;
                	});
                });
                ...
                "aButtons": [
                    {
                         ...
                         "fnClick": function ( nButton, oConfig, oFlash ) {
                             window.location.href = "#cert/add.htm";
                         },
                    ...
                    },
                    {
                        ...
                        "fnClick": function ( nButton, oConfig, oFlash ) {
                        	var certimgs = $("#certimgTable .data-check:checked");
                        	if ( certimgs.length == 0 ) {
					            layer.msg("请选择需要修改的资质信息", {time:2000, icon:5, shift:6});
                        	} else if ( certimgs.length > 1 ) {
                        		layer.msg("只能选择一条资质信息进行修改", {time:2000, icon:5, shift:6});
                        	} else {
                                var s = certimgs[0];
                                window.location.href = "#cert/edit/"+s.value+".htm";
                        	}
                        },
                        "fnComplete": function ( nButton, oConfig, oFlash, sFlash ) {
                        },
                       ...
                       "fnClick": function ( nButton, oConfig, oFlash ) {
                           ...
                           if ( certimgs.length == 0 ) {
                               layer.msg("请选择需要删除的资质信息", {time:1000, icon:5, shift:6});
                                ...
                                var param = {};
                                $.each(certimgs, function(i, n){
                                   param["ids["+i+"]"] = n.value;
                                });
                                
								layer.confirm('删除选择的资质信息, 是否继续?', {icon: 3, title:'提示'}, function(index){
									$.ajax({
										type : "POST",
										url  : "${APP_PATH}/system/cert/deletes.do",
										data : param,
										success : function(r) {
											if ( r.success ) {
												layer.msg("资质信息删除成功", {time: 1500, icon:1}, function(){
                                                    otable.draw();
                                                    $("#certimgTable #checkall")[0].checked = false;
												});
											} else {
												layer.msg("资质信息删除失败", {time: 1500, icon:2, shift: 6});
											}
										},
										error : function() {
											layer.msg("资质信息删除失败", {time: 1500, icon:2, shift: 6});

                ...
                "aButtons": [
                    {
                         ...
                         "fnClick": function ( nButton, oConfig, oFlash ) {
                             window.location.href = "#process/add.htm";
                         },
                    ...
                    },
                    {
                        ...
                        "fnClick": function ( nButton, oConfig, oFlash ) {
                        	...
                        	if ( processs.length == 0 ) {
					            layer.msg("请选择需要修改的流程信息", {time:2000, icon:5, shift:6});
                        	} else if ( processs.length > 1 ) {
                        		layer.msg("只能选择一条流程信息进行修改", {time:2000, icon:5, shift:6});
                        	} else {
                                var s = processs[0];
                                window.location.href = "#process/edit/"+s.value+".htm";
                        	}
                        },
                        "fnComplete": function ( nButton, oConfig, oFlash, sFlash ) {
                        },
                           ...
                           if ( processs.length == 0 ) {
                               layer.msg("请选择需要删除的流程信息", {time:1000, icon:5, shift:6});
                                ...
                                $.each(processs, function(i, n){
                                   //param["ids["+i+"]"] = n.value;
                                   param["processs["+i+"].id"] = n.value;
                                });
                                
								layer.confirm('删除选择的流程信息, 是否继续?', {icon: 3, title:'提示'}, function(index){
									$.ajax({
										type : "POST",
										...
										success : function(r) {
											if ( r.success ) {
												layer.msg("流程信息删除成功", {time: 1500, icon:1}, function(){
                                                    otable.draw();
                                                    $("#processTable #checkall")[0].checked = false;
												});
											} else {
												layer.msg("流程信息删除失败", {time: 1500, icon:2, shift: 6});
											}
										},
										error : function() {
											layer.msg("流程信息删除失败", {time: 1500, icon:2, shift: 6});
